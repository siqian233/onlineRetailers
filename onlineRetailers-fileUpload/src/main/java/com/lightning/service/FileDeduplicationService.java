...
public class FileDeduplicationService {
    ...
    public String processAndGetFileUrl(MultipartFile file) throws IOException, InterruptedException {
        ...
        if (StringUtils.hasText(existingFileUrl)) {
            log.info("Duplicate file detected (MD5: {}). Returning existing URL.", md5Hash);
            // 刷新过期时间
        ...
        if (Boolean.FALSE.equals(lockAcquired)) {
            ...
            log.warn("Failed to acquire lock for MD5: {}. Another thread is likely uploading. Waiting...", md5Hash);
            // 简单的轮询等待机制
            for (int i = 0; i < 5; i++) {
                TimeUnit.SECONDS.sleep(1);
                existingFileUrl = stringRedisTemplate.opsForValue().get(redisKey);
                if (StringUtils.hasText(existingFileUrl)) {
                    log.info("File URL found after waiting for MD5: {}", md5Hash);
                    return existingFileUrl;
                }
            }
            // 等待超时后仍未找到，抛出异常或返回错误提示
            throw new IOException("Failed to process file due to upload contention for MD5: " + md5Hash);
        }
        ...
        try {
            ...
            existingFileUrl = stringRedisTemplate.opsForValue().get(redisKey);
            if (StringUtils.hasText(existingFileUrl)) {
                log.info("Duplicate file detected after acquiring lock (MD5: {}).", md5Hash);
                return existingFileUrl;
            }

            // 5. 执行上传逻辑
            log.info("New unique file (MD5: {}). Uploading to OSS.", md5Hash);
            ...
            log.info("File uploaded to OSS: {}. MD5 stored in Redis.", newFileUrl);

            return newFileUrl;
        } finally {
            // 6. 释放锁
            stringRedisTemplate.delete(lockKey);
            log.debug("Lock released for MD5: {}", md5Hash);
        }
    }
}