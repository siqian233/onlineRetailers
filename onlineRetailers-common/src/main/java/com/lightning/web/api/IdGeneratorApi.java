...
public interface IdGeneratorApi {
     ...
     */
    @GetMapping("/id/batch")
    ResponseResult getBatchIds(@RequestParam(value = "size", defaultValue = "10") int size);

    /**
     * 解析时间戳
     */
    @GetMapping("/id/parse/time")
    ResponseResult parseTime(@RequestParam("id") long id);

    /**
     * 解析 datacenterId
     */
    @GetMapping("/id/parse/datacenter")
    ResponseResult parseDatacenter(@RequestParam("id") long id);

    /**
     * 解析 workerId
     */
    @GetMapping("/id/parse/worker")
    ResponseResult parseWorker(@RequestParam("id") long id);
